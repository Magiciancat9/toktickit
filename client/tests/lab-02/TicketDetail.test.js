...
describe("TicketDetail component", () => {
    beforeEach(() => {
        vi.restoreAllMocks();
        vi.spyOn(api, "fetchTicketByNumber").mockResolvedValue(MOCK_TICKET);
    });
    ...
    it("renders the active attachment with a download button and remove button", async () => {
        renderTicketDetail();
        await waitFor(() => expect(screen.getByTestId("active-attachments-list")).toBeInTheDocument());
    ...
    it("renders the removed attachment with REMOVED badge and no download button", async () => {
        renderTicketDetail();
        await waitFor(() => expect(screen.getByTestId("removed-attachments-list")).toBeInTheDocument());
    ...
    it("opens the remove modal when the Remove button is clicked", async () => {
        renderTicketDetail();
        await waitFor(() => expect(screen.getByTestId(`remove-btn-${ACTIVE_ATTACHMENT.id}`)).toBeInTheDocument());
