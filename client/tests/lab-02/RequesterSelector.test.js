...
describe("RequesterSelector component", () => {
    ...
    it("shows an error state when the API call fails", async () => {
        ...
        expect(screen.getByTestId("selector-error")).toHaveTextContent("Unable to load requesters");
    });
    it("shows an empty state when no active requesters exist", async () => {
        ...
        expect(screen.getByTestId("selector-empty")).toHaveTextContent("No active requesters found");
    });
    ...
    it("Continue button becomes enabled after selecting a requester", async () => {
        ...
        await userEvent.selectOptions(screen.getByTestId("requester-select"), "1" // Jennifer Anderson id
        );
