...
describe("CategoryList component", () => {
    ...
    it("shows an error state when the API call fails", async () => {
        ...
        expect(screen.getByTestId("categories-error")).toHaveTextContent("Unable to load categories. Please try again later.");
    });
    it("shows an error state when the API returns a non-200 response", async () => {
        vi.spyOn(api, "fetchCategories").mockRejectedValue(new Error("Failed to load categories: 500"));
        render(_jsx(CategoryList, {}));
        await waitFor(() => {
            expect(screen.getByTestId("categories-error")).toBeInTheDocument();
        });
